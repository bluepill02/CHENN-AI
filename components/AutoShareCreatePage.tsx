...
import { VehicleType } from "../utils/autoShareInterfaces";
import { chennaiLocations } from "../utils/mockRidesChennai";

const AutoShareCreatePage: React.FC = () => {
  const navigate = useNavigate();
  const { createRide, isUsingBackend, error: serviceError } = useAutoShare();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const timeISO = new Date(formData.time).toISOString();
      await createRide({
      ...
      setSubmitError(
        submitErr instanceof Error
          ? submitErr.message
          : 'Unable to create ride right now. Please retry in a moment.'
      );
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50">
      <div className="container mx-auto px-4 py-6 max-w-2xl">
        ...
        <div className="mb-6 bg-white/80 border border-amber-100 rounded-xl p-4 shadow-sm">
          ...
          <p className="text-xs text-amber-600">
            {serviceError
              ? serviceError
              : isUsingBackend
                ? 'Rides will be synced instantly with other commuters.'
                : 'Rides are stored locally and sync automatically once the backend is available.'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-xl p-8 space-y-6 border-l-4 border-amber-400">
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white py-4 rounded-lg font-bold text-lg transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl disabled:opacity-60 disabled:hover:scale-100"
            disabled={isSubmitting}
          >
            {isSubmitting ? 'Creating ride… / சவாரி உருவாக்கப்படுகிறது…' : 'Create Ride / சவாரி உருவாக்க'}
          </button>
